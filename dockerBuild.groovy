pipeline('DockerBuild') {
    ...
    parameters {
        ...
        booleanParam(name: 'deploy', defaultValue: true, description: '¿Realizar el despliegue de la imagen de Docker?')
        booleanParam(name: 'dbuild', defaultValue: true, description: '¿Construir la imagen de Docker?')
        string(name: 'dockerfilePath', defaultValue: '.', description: 'Ruta del archivo Dockerfile')
        string(name: 'dockercomposePath', defaultValue: '.', description: 'Ruta del archivo docker-compose')
        string(name: 'credentialsId', defaultValue: '', description: 'ID de las credenciales para el registro de Docker')
        booleanParam(name: 'pullToRegistry', defaultValue: false, description: '¿Realizar la carga de la imagen a un registro de Docker?')
    }
    stages {
        stage('Ejemplo de pipeline') {
            steps {
